package com.midefensa.midefensaapp.domain;

import jakarta.persistence.*;
import lombok.AllArgsConstructor;
import lombok.Builder;
import lombok.Data;
import lombok.NoArgsConstructor;

@Data
@AllArgsConstructor
@NoArgsConstructor
@Builder
@Entity
@Table(name = "tutela_derecho")
public class TutelaDerecho {
    private static final long serialVersionUID = 1L;

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Column(name = "tutela_derecho_id", nullable = false, unique = true)
    private Integer tutelaDerechoId;

    @ManyToOne
    @JoinColumn(name = "tutela_id", referencedColumnName = "tutela_id")
    private Tutela tutela;

    @ManyToOne
    @JoinColumn(name = "derecho_id", referencedColumnName = "derecho_id")
    private Derecho derecho;

    @Column(name = "estado", nullable = false, length = 1)
    private boolean estado;
}
